...
parser.add_argument('--adm1', default="Gyeongbuk",help='give adm1 level, default = Gyeongbuk')
parser.add_argument('--adm2', default="Yeongju-si",help='give adm2 level, default = "Yeongju-si')
parser.add_argument('--dir',default='Alphas_minor/',help='directory path for output .csv file')
...
alphas = pd.DataFrame(columns=["prov","minor","log_before",'notlog_before','log_after','notlog_after'],dtype='float')
new_alphaset = {"prov":my_province,"minor":my_city,'log_before':before_split[0],'notlog_before':before_split[1],'log_after':after_split[0],'notlog_after':after_split[1]}
alphas = alphas.append(new_alphaset,ignore_index = True)
if not os.path.isdir(result_dir):
	os.mkdir(result_dir)
alphas.to_csv(result_dir+my_province+'_'+my_city+".csv",mode='w')
